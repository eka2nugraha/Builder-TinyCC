#ifndef dL_2601
#define dL_2601
static const char L_2601[]="1, 2, 4 or 8 constant";
static const char L_2602[]="register";
static const char L_2603[]="unknown register %%%s";
static const char L_2604[]=")";
static const char L_2605[]="can't encode register %%%ch when REX prefix is required";
static const char L_2606[]="acdb";
static const char L_2607[]="%3d: %08x\n";
static const char L_2608[]="size=%d nb=%d f0=%d f1=%d f2=%d f3=%d\n";
static const char L_2609[]="incorrect number of operands";
static const char L_2610[]="incorrect prefix";
static const char L_2611[]="segment prefix must be followed by memory reference";
static const char L_2612[]="cmovzl";
static const char L_2613[]="bad operand with opcode '%s'";
static const char L_2614[]="wlq";
static const char L_2615[]="cmov";
static const char L_2616[]="unknown opcode '%s'";
static const char L_2617[]="cannot infer opcode suffix";
static const char L_2618[]="bad op table";
static const char L_2619[]="invalid displacement";
static const char L_2620[]="cannot relocate";
static const char L_2621[]="cannot relocate";
static const char L_2622[]="unknown constraint '%c'";
static const char L_2623[]="%reg";
static const char L_2624[]="invalid reference in constraint %d ('%s')";
static const char L_2625[]="cannot reference twice the same operand";
static const char L_2626[]="asm regvar requests register that's taken already";
static const char L_2627[]="'%c' modifier can only be applied to outputs";
static const char L_2628[]="asm constraint %d ('%s') could not be satisfied";
static const char L_2629[]="could not find free output register for reloading";
static const char L_2630[]="%%%d [%s]: \"%s\" r=0x%04x reg=%d\n";
static const char L_2631[]="";
static const char L_2632[]="out_reg=%d\n";
static const char L_2633[]="L.42";
static const char L_2634[]="%d";
static const char L_2635[]="(%rip)";
static const char L_2636[]="%d(%%rbp)";
static const char L_2637[]="%d(%%ebp)";
static const char L_2638[]="internal compiler error";
static const char L_2639[]="(%%%s)";
static const char L_2640[]="internal compiler error";
static const char L_2641[]="cannot use byte register";
static const char L_2642[]="cannot use byte register";
static const char L_2643[]="%%%s";
static const char L_2644[]="memory";
static const char L_2645[]="cc";
static const char L_2646[]="flags";
static const char L_2647[]="invalid clobber register '%s'";
#endif
/*
...
static void parse_operand(TCCState *s1, Operand *op)
{
    ...
    if (tok == '%') {
        ...
        reg_error:
            tcc_error(L_2603, get_tok_str(tok, &tokc));
        }
        next();
    no_skip: ;
    } else if (tok == '$') {
...
again:
	...
	int it = pa->instr_type & OPCT_MASK;
	    ...
	    /* cmovxx is a test opcode but accepts multiple sizes.
	       ...
	       variants below when we don't find e.g. L_2612.  */
    ...
    if (pa->sym == 0) {
	    ...
	    if (ts->len >= 6
		&& strchr(L_2614, ts->str[ts->len-1])
		&& !memcmp(ts->str, L_2615, 4)) {
		opcode = tok_alloc(ts->str, ts->len-1)->tok;
		goto again;
	    }
            tcc_error(L_2616, ts->str);
        }
    }
...
/* If T (a token) is of the form L_2623 returns the register
                                    ...
                                    int *pout_reg)
{
    ...
    *pout_reg = -1;
    ...
    for(i=0;i<nb_operands;i++) {
        ...
        printf(L_2630,
               j,
               op->id ? get_tok_str(op->id, NULL) : L_2631,
               op->constraint,
               op->vt->r,
               op->reg);
    }
    if (*pout_reg >= 0)
        printf(L_2632, *pout_reg);
#endif
}
                              ...
                              SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
		...
		/* In case of anonymous symbols (L_2633, used
        ...
        if (modifier == 'n')
            val = -val;
        snprintf(buf, sizeof(buf), L_2634, (int)sv->c.i);
        cstr_cat(add_str, buf, -1);
    no_offset:;
#ifdef TCC_TARGET_X86_64
        if (r & VT_LVAL)
            cstr_cat(add_str, L_2635, -1);
#endif
    } else if ((r & VT_VALMASK) == VT_LOCAL) {
#ifdef TCC_TARGET_X86_64
        snprintf(buf, sizeof(buf), L_2636, (int)sv->c.i);
#else
        snprintf(buf, sizeof(buf), L_2637, (int)sv->c.i);
#endif
        cstr_cat(add_str, buf, -1);
    } else if (r & VT_LVAL) {
        reg = r & VT_VALMASK;
        if (reg >= VT_CONST)
            tcc_error(L_2638);
        snprintf(buf, sizeof(buf), L_2639,
#ifdef TCC_TARGET_X86_64
        ...
        snprintf(buf, sizeof(buf), L_2643, get_tok_str(reg, NULL));
        cstr_cat(add_str, buf, -1);
    }
}
